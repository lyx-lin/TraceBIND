## calculate footprintings by negative binomial method, 
## Tn5Insertion, pred_bias, and positions should be consecutive and matched 
## the option for p.adjust.method includes Benjamini-Hochberg / Benjamini-Yekutieli, according to hypothesis
## output: adjusted pval and effect size matrix, where each row is a base pair position and each column is a center window. 
library(Rcpp)
sourceCpp("/home/mnt/weka/nzh/team/woodsqu2/codes/footprints/nbinomial_single.cpp")
...
NB_footprintings = function(Tn5Insertion,
  ...
  results = pbmcapply::pbmcmapply(function(position_index) {
...
### remove regions that overlap with the reported binding_sites
...
### the same with searching binding sites by pvalue
binding_sites_pval = function(p_value_matrix, 
                              # thresholds = NULL, 
                              smoothed = FALSE, 
                              pval_threshold = 0.1, ### pval threshold reported binding sites
                              width_threshold = 10){ ### width threshold reported binding sites
  ...
  log_p_value <- t(log_p_value) %>%
    ...
    tidyr::pivot_longer(-width, names_to = "position", values_to = "minus_log_p_value")
...
### the same with searching binding sites by pvalue, 
### expect searching for maximum effect size
binding_sites_effect_size = function(p_value_matrix, 
                                     effect_size_matrix, 
                                     pval_threshold = 0.1, 
                                     width_threshold = 10){
  ...
  # effect_size_matrix[p_value_matrix > pval_threshold & 
  ...
  # binding_site_candidates[c('width_smoothed', 'position_smoothed', 'pval_smoothed')] = 
  #   data.table::rbindlist(lapply(1:dim(binding_site_candidates)[1], 
  #                               function(x){
   #                                logpp_value_range = log_p_value[log_p_value$position <= binding_site_candidates$position[x] + 
  #                                                                   log_p_value$width[x]/4 & 
  #                                                                   log_p_value$position >= binding_site_candidates$position[x] - 
  #                                                                   binding_site_candidates$width[x]/4 &
  #                                                                   log_p_value$width >= max(10, binding_site_candidates$width[x]/2) & 
  #                                                                   log_p_value$width <= binding_site_candidates$width[x]*2, 
  #                                                                        ]
  #                                 c(logpp_value_range[which.max(logpp_value_range$p_value), 1:3])
   #                                
   #                              }
  #   ))
  ...
  binding_site_candidates$position_effect_size = as.numeric(binding_site_candidates$position_effect_size)
  binding_site_candidates$width_effect_size = as.numeric(binding_site_candidates$width_effect_size)
  return(binding_site_candidates)
}

### compare binding events between different conditions
